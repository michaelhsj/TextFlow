...
Builds the TextFlow Dagster job image for linux/amd64 and pushes it to Artifact
Registry. The resulting image matches the path Terraform expects:
  REGION-docker.pkg.dev/PROJECT_ID/textflow-jobs/pipeline:TAG
...
IMAGE="${REGION}-docker.pkg.dev/${PROJECT_ID}/textflow-jobs/pipeline:${TAG}"
